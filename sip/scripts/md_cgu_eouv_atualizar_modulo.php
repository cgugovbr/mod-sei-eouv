...
{
  private $nomeModulo = 'EOUV - Integraчуo com sistema FalaBR (E-ouv)';
  ...
  /**
   * 1. Comeчamos a contralar a partir da versуo 2.0.5 que щ a њltima estсvel para o SEI 3.0
   * 2. A versуo 3.0.0 comeчa a utilizar a versуo REST dos webservices do E-Ouv
   * 3. A versуo 4.0.0 importa manifestaчѕes do tipo 8 (acesso р informaчуo) que sуo oriundas antigo e-Sic integrado
   * ao FalaBR, esta versуo importa tambem os recursos de 1Њ e 2Њ instтncia
   */
  ...
  protected function instalarv205(){
    ...
    if ($objSistemaDTO == null) {
      throw new InfraException('Sistema SEI nуo encontrado.');
    }
    ...
    if ($objPerfilDTO == null) {
      throw new InfraException('Perfil Administrador do sistema SEI nуo encontrado.');
    }
    ...
    $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_relatorio_importacao', 'Relatѓrio de importaчуo de manifestaчѕes do EOUV',
      'controlador.php?acao=md_cgu_eouv_relatorio_importacao');
    $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_depara_importacao_consultar', 'Consulta a tabela DePara referente a importaчуo de Tipo de manifestaчуo',
      'controlador.php?acao=md_cgu_eouv_depara_importacao_consultar');
    $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_depara_importacao_excluir', 'Excluir item da tabela DePara referente a importaчуo de Tipo de manifestaчуo',
    ...
    $numIdRecursoIntegracaoSei = $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_integracao_sei', 'Integraчуo entre E-ouv e SEI',
      'controlador.php?acao=md_cgu_eouv_integracao_sei');

    $this->logar('Valor id objeto' . $numIdRecursoIntegracaoSei);

    $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_relatorio_importacao_detalhar', 'Relatѓrio Detalhado de importaчуo de manifestaчѕes do EOUV',
    ...
    $numIdRecursoRelatorioImportacaoEouvSei = $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_relatorio_importacao_listar', 'Relatѓrio de importaчуo de manifestaчѕes do EOUV',
    ...
    if ($objMenuDTO == null) {
      throw new InfraException('Menu do sistema SEI nуo encontrado.');
    }
    ...
    $menuEouv = $this->adicionarItemMenu($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      ...
      'Integraчуo entre E-ouv e SEI', 1100);

    $this->adicionarItemMenu($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      $numIdMenuSei, $menuEouv->getNumIdItemMenu(),
      $numIdRecursoRelatorioImportacaoEouvSei, 'Importaчуo de Manifestaчуo', 'Relatѓrio de Importaчуo de Manifestaчуo', 10);
  }
  ...
  protected function instalarv300()
  {
    ...
    $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_parametro', 'Controle de Parтmetros mѓdulo SEI x E-ouv',
      'controlador.php?acao=md_cgu_eouv_parametro');

    $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_parametro_consultar', 'Consulta de Parтmetros mѓdulo SEI x E-ouv',
    ...
    $numIdRecursoParametro = $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_parametro_listar', 'Lista de Parтmetros mѓdulo SEI x E-ouv',
      'controlador.php?acao=md_cgu_eouv_parametro_listar');

    $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_parametro_cadastrar', 'Cadastro de Parтmetros mѓdulo SEI x E-ouv',
      'controlador.php?acao=md_cgu_eouv_parametro_cadastrar');

    $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_parametro_alterar', 'Alteraчуo de Parтmetros mѓdulo SEI x E-ouv',
    ...
    $this->logar('CRIANDO e VINCULANDO ITEM MENU A PERFIL - E-Ouv->Parтmetros');

    $this->adicionarItemMenu($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      $numIdMenuSei, $objItemMenuDTOEouv->getNumIdItemMenu(),
      $numIdRecursoParametro, 'Parтmetros do Mѓdulo E-ouv', 'Parтmetros', 20);
  }
  ...
  protected function instalarv400()
  {
    ...
    $numIdRecursoParametro = $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_parametro_listar_esic', 'Lista de Parтmetros e-Sic',
    ...
    $this->adicionarItemMenu($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      $numIdMenuSei, $objItemMenuDTOEouv->getNumIdItemMenu(),
      $numIdRecursoParametro, 'Parтmetros do Mѓdulo e-Sic', 'Parтmetros', 30);

    // Recursos e menu para ediчуo do De Para entre tipos FalaBR e Processo
    $this->logar('ADICIONANDO MENU DE CONFIGURAЧУO DE TIPOS DE MANIFESTAЧУO E PROCESSOS');
    $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_depara_importacao_alterar', 'Alterar uma Associaчуo entre Tipo de Manifestaчуo FalaBR e Processo',
    ...
    $numIdRecursoDeParaListar = $this->adicionarRecursoPerfil($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      'md_cgu_eouv_depara_importacao_listar', 'Listar Associaчѕes entre o Tipo de Manifestaчуo FalaBR e Processo',
      'controlador.php?acao=md_cgu_eouv_depara_importacao_listar');
    $this->adicionarItemMenu($numIdSistemaSei, $numIdPerfilSeiAdministrador,
      $numIdMenuSei, $objItemMenuDTOEouv->getNumIdItemMenu(),
      $numIdRecursoDeParaListar, 'Tipos de Manifestaчуo', 'Associaчѕes entre o Tipo de Manifestaчуo no Falabr e o Tipo de Processo', 40);
  }
  ...
  protected function instalarv402(){
      ...
      $this->logar('EXCLUINDO COLUNA id_md_eouv_rel_import DA TABELA md_eouv_rel_import');
      ...
      $tarefa = $infraAgendamentoTarefaRN->consultar();
      $numRegistros = count($tarefa);
      if($numRegistros == 0) {
          $infraAgendamentoTarefaDTO->setStrComando('MdCguEouvAgendamentoRN::executarImportacaoManifestacaoESic');
          $tarefa = $infraAgendamentoTarefaRN->consultar($infraAgendamentoTarefaDTO);
      }
      $numRegistros = count($tarefa);
      if($numRegistros == 0) {
          $infraAgendamentoNovaTarefaDTO = new InfraAgendamentoTarefaDTO();
          $infraAgendamentoNovaTarefaDTO->setStrDescricao('Rotina responsсvel pela execuчуo da importaчуo de manifestaчѕes cadastradas no FalaBR que serуo importadas para o SEI/SUPER como um novo processo. Se baseia na data da њltima execuчуo com sucesso atщ a data atual.');
          ...
          $infraAgendamentoNovaTarefaDTO->setStrStaPeriodicidadeExecucao('N');
          $infraAgendamentoNovaTarefaDTO->setStrPeriodicidadeComplemento('0,10,20,30,40,50');
      ...
      //TODO preciso apagar os agendamentos anteriores?
